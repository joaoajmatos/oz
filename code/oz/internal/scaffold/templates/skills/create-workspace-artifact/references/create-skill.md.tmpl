...
4. Fill in `## When to invoke` and `## Steps`. Do not leave placeholder text.
5. If the skill has multiple execution paths, create `skills/<name>/references/` and add one
   markdown file per path. Reference each file explicitly in `## Steps`.
6. If the skill uses templates or support files, add them to `skills/<name>/assets/`.
7. Register the skill in the authorizing agent's `AGENT.md` under `## Skills`:
   `- \`skills/<name>/\` — <one-line description>`
8. Verify: confirm `SKILL.md` has both required sections and that all referenced files exist.

### Tweaking an existing skill

1. Open `skills/<name>/SKILL.md`.
2. Make the targeted change.
3. Confirm `## When to invoke` and `## Steps` are still present and accurate.
4. If references or assets changed, update the relevant files in `references/` or `assets/`.
5. Verify: confirm all files referenced in `SKILL.md` still exist and are up to date.
