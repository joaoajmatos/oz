...
- **Tier B — code↔spec drift** (Go, graph-backed today): spec mentions of `code/...` paths or
  exported-identifier-shaped tokens that no longer resolve against `code_symbol` nodes in
  `graph.json`; exported symbols never mentioned in scanned markdown. Pluggable extractors
  for other languages remain V2 (A-14).
...
| A-07 | As `oz audit drift`, I detect spec mentions of code that no longer exists, and exported Go symbols specs never mention | `DRIFT001` (error): a `code/...` path mentioned under scanned markdown does not exist on disk. `DRIFT002` (error): a backticked identifier candidate (default: exported Go shape, underscores excluded — see §6) is not in the graph `code_symbol` set. `DRIFT003` (warn): an exported symbol from the graph is never mentioned as an identifier candidate in scanned specs/docs. |
...
| A-11 | As a developer, I can include test files in drift extraction with `--include-tests` | By default `code_symbol` entries come from the graph (production `*.go` only). With the flag, drift merges exported symbols parsed from `*_test.go` under `code/` using the same Go indexer as `context build`. |
...
```mermaid
flowchart TB
  cmd["cmd/audit.go parent and subcommands"]
  core["internal/audit/audit.go Finding Report RunAll"]
  cmd --> core
  core --> orphans["orphans graph only"]
  core --> coverage["coverage graph and disk"]
  core --> staleness["staleness rebuild and overlay"]
  core --> drift["drift graph symbols and specscan"]
```
...
| `DRIFT002` | error | drift | spec text mentions an identifier candidate not present in the graph symbol set |
...
### Drift symbol source (Tier B)

Exported Go symbols for drift are read from **`code_symbol` nodes** in `context/graph.json`,
produced during `oz context build` by `internal/codeindex/goindexer` (`go/parser`, same
pipeline as structural indexing). The in-memory type is `drift.Symbol` (`Pkg`, `Name`,
`Kind`, `File`, `Line`). See ADR `specs/decisions/0001-audit-v1-symbols-from-graph-codeindex.md`.

`DRIFT004` / `DRIFT900` from the catalogue below are **not implemented** in V1.

### Spec scanner

`specscan` walks `specs/` (and `docs/` when `--include-docs` is set) and extracts:

- backticked tokens that match **`DefaultGoExportedIdentPattern`** in code (default: exported
  Go identifiers in unqualified or `pkg.Symbol` form, **underscores excluded** so env-style
  `ALL_CAPS` tokens are not candidates — AT-01 / Sprint A5-05). Callers may OR additional
  compiled patterns via `specscan.Options.IdentPatterns` (nil/empty keeps the default alone).
- markdown link targets and backtick contents starting with `code/`.

For each candidate it records the source file and line. The drift orchestrator cross-references
identifier candidates against the symbol set and path candidates against the filesystem to
produce `DRIFT001`–`DRIFT003`.

False-positive control: the default pattern is intentionally narrower than “any PascalCase
word”. Common English capitals can still match valid Go exports (`Build`, etc.); `DRIFT002`
only fires when the token is not in the graph symbol set. Residual noise is expected to be
low on real repos; the oz workspace self-validates to **zero** `DRIFT002` after the AT-01
narrowing (`docs/open-items.md`).
...
- **`go/parser` (via context code indexing) over tree-sitter in V1.** Zero new dependency for
  Go symbol extraction; symbols live in `graph.json` so audit does not re-parse production
  sources by default. Tree-sitter remains a sensible V2 once multi-language indexing ships.
