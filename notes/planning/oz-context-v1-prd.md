...
| C-08 | As an LLM, routing packets from `oz context query` are enriched with semantic concepts when `context/semantic.json` exists | When a semantic overlay is present, `relevant_concepts` lists **query-relevant** reviewed concept names (ranked by the retrieval concept scorer, capped; see `specs/routing-packet.md`). If no overlay, no field — no error. |
