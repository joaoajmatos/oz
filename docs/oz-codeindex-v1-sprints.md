...
| CI-1-04 | Integrate code indexing pass in `internal/context/builder.go` | After markdown indexing, builder calls `WalkCode(..., []Indexer{goindexer.New()})`; appends nodes/edges before normalize; build remains deterministic. |
